...
const (
	...
	createActionHistory    = "CREATE TABLE IF NOT EXISTS %s " +
		...
		"amount DECIMAL(65, 0) NOT NULL, receipt_status TEXT NOT NULL, UNIQUE KEY %s (`from`), UNIQUE KEY %s (`to`), PRIMARY KEY (action_hash), FOREIGN KEY (block_height) REFERENCES %s(block_height))"
