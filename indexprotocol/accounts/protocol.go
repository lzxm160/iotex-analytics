...
func (p *Protocol) CreateTables(ctx context.Context) error {
	...
	if _, err := p.Store.GetDB().Exec(fmt.Sprintf(createBalanceHistory,
		BalanceHistoryTableName)); err != nil {
		return err
	}
	if _, err := p.Store.GetDB().Exec(fmt.Sprintf(createAccountInflow,
		AccountInflowTableName,EpochAddressIndexName)); err != nil {
		return err
	}
	if _, err := p.Store.GetDB().Exec(fmt.Sprintf(createAccountOutflow,
		AccountOutflowTableName,EpochAddressIndexName)); err != nil {
		return err
	}
	if _, err := p.Store.GetDB().Exec(fmt.Sprintf(createAccountIncome,
		AccountIncomeTableName,EpochAddressIndexName)); err != nil {
		return err
	}
	return nil
}
