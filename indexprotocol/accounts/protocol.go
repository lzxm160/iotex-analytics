...
func (p *Protocol) CreateTables(ctx context.Context) error {
	...
	if _, err := p.Store.GetDB().Exec(fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s "+
		...
		"address VARCHAR(41) NOT NULL, `in` DECIMAL(65, 0) DEFAULT 0, `out` DECIMAL(65, 0) DEFAULT 0, PRIMARY KEY (block_height,action_hash,address))", AccountHistoryTableName)); err != nil {
		return err
	}
